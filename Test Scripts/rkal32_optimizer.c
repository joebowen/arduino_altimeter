/*
NAME
rkal32_optimizer.c - A third order Kalman filter for
RDAS interpreted data files, modified for use with optimizer.py.

SYNOPSIS
rkal32 infile outfile alt_var accel_var model_var
...
main(int argc, char **argv)
{
    ...
    altitude_variance = atof(argv[3])*atof(argv[3]);
    acceleration_variance = atof(argv[4])*atof(argv[4]);
    model_variance = atof(argv[5])*atof(argv[5]);
    ...
    */
    ...
    fprintf(ofp, "Input noise values used (standard deviation):\n");
    fprintf(ofp, "#Altitude - %15f meters\n", sqrt(altitude_variance));
    fprintf(ofp, "#Acceleration - %15f meters/sec/sec\n", sqrt(acceleration_variance));
    fprintf(ofp, "#Model noise - %15f meters/sec/sec\n\n", sqrt(model_variance));
    fprintf(ofp, "#Kalman gains converged after %d iterations.\n", k);

    for( i = 0; i <= 2; i++)
    {
        for( j = 0; j <= 1; j++)
        {
            fprintf(ofp, "%15f ", kgain[i][j]);
        }
        fprintf(ofp, "\n");
    }

    fprintf(ofp, "\n");
    fprintf(ofp, "#Estimated output first order statistics (standard deviation):\n");
    fprintf(ofp, "#Altitude - %15f meters\n", sqrt(pest[0][0]));
    fprintf(ofp, "#Velocity - %15f meters/sec\n", sqrt(pest[1][1]));
    fprintf(ofp, "#Acceleration - %15f meters/sec/sec\n", sqrt(pest[2][2]));
    ...
    */
    ...
    while(fscanf(ifp, "%lf,%lf,%lf\n", &time, &accel, &pressure) != EOF)
    {
        ...
        */
        ...
        /* Output */
        fprintf(ofp, "%15f,%15f,%15f,%15f,%15f,%15f\n", time, pressure, accel, est[0], est[1], est[2]);

        sum_alt_diff += fabs(est[0] - pressure);
        count++;
