...
{   
  ...
  write_to_sd("time(ms),temperature(C),pressure,", NONEWLINE);
  write_to_sd("absolute altitude(m)(probably wrong),altitude(m),raw velocity(m/s),", NONEWLINE);
  write_to_sd("Raw X Accel(m/s^2),Raw Y Accel(m/s^2),Raw Z Accel(m/s^2),Vector Accel(m/s^2)", NONEWLINE);
  write_to_sd(" ", NEWLINE);
...
void altimeter_state_tree(unsigned long time, double accelVel, double vel, double alt)
{
  switch (state)
  {
    case S_INIT:
